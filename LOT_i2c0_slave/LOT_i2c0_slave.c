...
ISR( TWI_vect )
{
    switch( TWSR )
    {
        ...
        case SLAVE_NACK_AFTER_MASTER_DATA:
            ...
            { slave_register[slave_register_address] = TWDR; }
            break;
        case I2C_BUS_ERROR: TWCR |= _BV( TWSTO ) | _BV( TWINT ) | _BV( TWEN ) | _BV( TWIE ); break;
        default: SLAVE_NACK_RESPONSE; break;
    }
}
...
{
    if( reg_address < LOT_I2C0_SLAVE_REGISTER_LENGTH ) { return slave_register[reg_address]; }
    else
    {
        return -1;
    }
...
{
    if( reg_address < LOT_I2C0_SLAVE_REGISTER_LENGTH ) { slave_register[reg_address] = data; }
}