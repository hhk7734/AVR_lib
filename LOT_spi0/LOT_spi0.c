...
void LOT_spi0_begin( const LOT_spi0_clk_divider_typedef clock_divider )
{
    ...
    SPCR = _BV( SPE ) | ( LOT_SPI0_DATA_ORDER << DORD ) | _BV( MSTR )
           | ( LOT_SPI0_CLK_IDLE_STATE << CPOL ) | ( LOT_SPI0_CLK_SAMPLING_EDGE << CPHA );

    /// 통신 속도 설정
    SPSR &= ~_BV( SPI2X );
    switch( clock_divider )
    {
        case LOT_SPI0_SCK_DIV_2: SPSR |= _BV( SPI2X ); break;
        case LOT_SPI0_SCK_DIV_4: break;
        case LOT_SPI0_SCK_DIV_8:
            SPSR |= _BV( SPI2X );
            SPCR |= _BV( SPR0 );
            break;
        case LOT_SPI0_SCK_DIV_16: SPCR |= _BV( SPR0 ); break;
        case LOT_SPI0_SCK_DIV_32:
            SPSR |= _BV( SPI2X );
            SPCR |= _BV( SPR1 );
            break;
        case LOT_SPI0_SCK_DIV_64: SPCR |= _BV( SPR1 ); break;
        case LOT_SPI0_SCK_DIV_128: SPCR |= _BV( SPR1 ) | _BV( SPR0 ); break;
    }
}
 ...
 */
LOT_status_typedef LOT_spi0_transmit_receive_array( volatile uint8_t *port,
                                                    const uint8_t     pin,
                                                    uint8_t *         buf,
                                                    int8_t            count )
{
 ...
 * @return LOT_status_typedef
 */
LOT_status_typedef LOT_spi0_write_register( volatile uint8_t *port,
                                            const uint8_t     pin,
 ...
 */
uint8_t
    LOT_spi0_read_register( volatile uint8_t *port, const uint8_t pin, const uint8_t reg_address )
{
    *port &= ~_BV( pin );
#if LOT_SPI0_TIME_OUT <= 255
    uint8_t time_out;
 ...
 * @return LOT_status_typedef
 */
LOT_status_typedef LOT_spi0_read_successive_register( volatile uint8_t *port,
                                                      const uint8_t     pin,
                                                      const uint8_t     reg_address,
                                                      uint8_t *         buf,
                                                      int8_t            count )
