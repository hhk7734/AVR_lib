 ...
 */
static uint8_t LOT_dec_to_bcd( uint8_t dec ) { return ( ( dec / 10 ) << 4 ) | ( dec % 10 ); }
 ...
 */
LOT_status_typedef LOT_ds3231_set_time( const uint8_t _xx_year,
                                        const uint8_t _month,
                                        const uint8_t _date,
                                        const uint8_t _day,
                                        const uint8_t _24_hour,
                                        const uint8_t _minute,
 ...
 * @return LOT_status_typedef
 */
LOT_status_typedef LOT_ds3231_get_time( uint8_t *_xx_year,
                                        uint8_t *_month,
                                        uint8_t *_date,
                                        uint8_t *_day,
                                        uint8_t *_24_hour,
...
{
    uint8_t buf[7];
    if( LOT_i2c0_read_successive_register( LOT_DS3231_ADDRESS, LOT_DS3231_SECOND, buf, 7 )
        != LOT_OK )
    { return LOT_ERROR; }
 ...
 * @return LOT_status_typedef
 */
LOT_status_typedef LOT_ds3231_write_time_register( const uint8_t _time_register,
                                                   const uint8_t _time )
{
    if( _time_register > LOT_DS3231_YEAR ) { return LOT_ERROR; }
...
{
    if( _time_register > LOT_DS3231_YEAR ) { return -1; }
