...
void EtherCard::copyIp( uint8_t *dst, const uint8_t *src ) { memcpy( dst, src, 4 ); }

void EtherCard::copyMac( uint8_t *dst, const uint8_t *src ) { memcpy( dst, src, 6 ); }

void EtherCard::printIp( const char *msg, const uint8_t *buf )
{
    uart1.transmit( msg );
    EtherCard::printIp( buf );
    uart1.transmit_CR_NL();
}
...
void EtherCard::printIp( const uint8_t *buf )
{
    ...
    {
        uart1.transmit( static_cast<uint16_t>( buf[i] ) );
        if( i < 3 ) uart1.transmit( '.' );
    }
}
...
{
    hstr[1] = ( c & 0xf ) + '0';
    if( ( c & 0xf ) > 9 ) { hstr[1] = ( c & 0xf ) - 10 + 'a'; }
    c       = ( c >> 4 ) & 0xf;
    hstr[0] = c + '0';
    if( c > 9 ) { hstr[0] = c - 10 + 'a'; }
    hstr[2] = '\0';
}
...
uint8_t EtherCard::parseIp( uint8_t *bytestr, char *str )
{
    ...
    while( *str && i < 4 )
    {
        ...
        if( sptr == NULL && isdigit( *str ) ) { sptr = str; }
        if( *str == '.' )
        {
            *str       = '\0';
...
void EtherCard::makeNetStr(
    char *resultstr, uint8_t *bytestr, uint8_t len, char separator, uint8_t base )
{
    uint8_t i = 0;
    uint8_t j = 0;
    while( i < len )
