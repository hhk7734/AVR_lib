...
typedef void ( *UdpServerCallback )( uint16_t    dest_port,    ///< Port the packet was sent to
...
typedef void ( *DhcpOptionCallback )( uint8_t        option,    ///< The option number
...
public:
    ...
    virtual void transmit_basic( uint8_t b ) { put( b ); }
...
*
*/
class BufferFiller : public LOT_transmit {
...
public:
    ...
    */
    virtual void transmit_basic( uint8_t v ) { *ptr++ = v; }
};
...
public:
    ...
    static uint8_t  destination_ip[4];     ///< hispip, DNS lookup result
    static uint16_t destination_port;      ///< hisport, TCP port to connect to (default 80)
    ...
    static uint16_t
        delaycnt;    ///< Counts number of cycles of packetLoop when no packet received - used to trigger periodic gateway ARP request
    ...
    */
    static void browseUrl( const char *urlbuf,
    ...
    */
    ...
    /**
     * @brief 송신을 위한 UDP 메시지 준비
     * @param uint16_t source_port
     * @param uint8_t *_destination_ip 목적지 IPv4 주소 배열
     * @param uint16_t _destination_port
     */
    static void udpPrepare( uint16_t sport, const uint8_t *dip, uint16_t dport );

    /**
     * @brief UDP 패킷 송신
     * @param uint16_t size payload 크기
     */
    ...
    */
    static void sendUdp(
        const char *data, uint8_t len, uint16_t sport, const uint8_t *dip, uint16_t dport );
    ...
    */
    ...
    /**
     * @brief DHCP 설정 업데이트
     * @param uint16_t size 수신된 데이터 크기
     */
    ...
    */
    ...
    /**
     * @brief DHCP 네트워크 인터페이스 설정
     * @param char *_host_name 호스트 이름 설정, 10글자
     * @return bool 설정 성공 시 true
     */
    ...
    *     @note   Result is stored in <i>destination_ip</i> member
    */
    ...
    */
    static void
        makeNetStr( char *resultstr, uint8_t *bytestr, uint8_t len, char separator, uint8_t base );
};
