...
static void dnsRequest( const char *hostname, bool fromRam )
{
    ...
    if( ether.dnsip[0] == 0 ) memset( ether.dnsip, 8, 4 );    // use 8.8.8.8 Google DNS as default
...
/** @brief  Check if packet is DNS response.
    ...
    @note   destination_ip contains IP address of requested host or 0.0.0.0 on failure
*/
static bool checkForDnsAnswer( uint16_t plen )
{
    ...
    if( plen < 70 || gPB[UDP_SRC_PORT_L_P] != 53 ||    //from DNS source port
        ...
        ||                                      //response to same port as we sent from (MSB)
    ...
    if( ( p[3] & 0x0F ) != 0 ) return true;     //DNS response recieved with error
    ...
    for( ;; )
    {
        ...
        if( p + 14 > gPB + plen ) break;
        if( p[1] == 1 && p[9] == 4 )
        {    // type "A" and IPv4
            ether.copyIp( ether.destination_ip, p + 10 );
            break;
        }
        p += p[9] + 10;
    }
...
bool EtherCard::dnsLookup( const char *name, bool fromRam )
{
    ...
    while( !isLinkUp() )
    {
        if( ( uint16_t )( time.millis() - start ) >= 30000 )
    ...
    memset( destination_ip, 0, 4 );
    dnsRequest( name, fromRam );

    start = time.millis();
    while( destination_ip[0] == 0 )
    {
        if( ( uint16_t )( time.millis() - start ) >= 30000 )
        ...
        if( len > 0 && packetLoop( len ) == 0 )    //packet not handled by tcp/ip packet loop
            if( checkForDnsAnswer( len ) ) return false;    //DNS response recieved with error
    }
