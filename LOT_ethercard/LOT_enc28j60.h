// Microchip LOT_enc28j60 Ethernet Interface Driver
// Author: Pascal Stang
...
//   CS     -    Pin PA8
//-----------------------------------------------------------------
#ifndef _LOT_ENC28J60_H_
#define _LOT_ENC28J60_H_

#include "LOT_time.h"
#include "LOT_spi1.h"

/** This class provide low-level interfacing with the LOT_enc28j60 network interface. This is used by the EtherCard class and not intended for use by (normal) end users. */
class LOT_enc28j60 {
public:
    ///
    static uint8_t  buffer[];      ///< buffer는 송수신 공유
    ...
    */
    ...
    /**
     * @brief 수신 패킷을 buf에 저장
     * @return uint16_t 패킷 크기
     */
    ...
    */
    static uint8_t doBIST( void );
};

typedef LOT_enc28j60 Ethernet;    //!< Define alias Ethernet for ENC28J60
