...
void Stash::freeBlock( uint8_t block ) { bitSet( map[block >> 3], block & 7 ); }

uint8_t Stash::fetchByte( uint8_t blk, uint8_t off )
{
    return blk == bufs[0].bnum
               ? bufs[0].bytes[off]
               : blk == bufs[1].bnum ? bufs[1].bytes[off] : ether.peekin( blk, off );
}

void Stash::initMap( uint8_t last )
{
    while( --last > 0 ) freeBlock( last );
}
...
void Stash::load( uint8_t idx, uint8_t blk )
{
    if( blk != bufs[idx].bnum )
    {
        if( idx == 0 )
        {
            ...
            if( blk == bufs[1].bnum ) bufs[1].bnum = 255;    // forget read page if same
        }
...
{
    load( 1, curr );
    if( curr == last && offs >= bufs[1].tail ) return 0;
...
uint16_t Stash::size() { return 63 * count + fetchByte( last, 62 ) - sizeof( StashHeader ); }
    ...
    for( ;; )
    {
        ...
        if( c == '$' )
        {
            ...
            switch( pgm_read_byte( fmt++ ) )
            {
                ...
                case 'S': arglen = strlen( ( const char * )argval ); break;
                case 'F': arglen = strlen_P( ( PGM_P )argval ); break;
                case 'E':
                {
...
void Stash::extract( uint16_t offset, uint16_t count, void *buf )
{
    ...
    for( uint16_t i = 0; i < offset + count; )
    {
        ...
        switch( mode )
        {
            ...
            case 'D':
            case 'S': c = *ptr++; break;
            case 'F': c = pgm_read_byte( ptr++ ); break;
            case 'E':
                //c = eeprom_read_byte((byte*) ptr++);
                break;
            case 'H': c = ( ( Stash * )ptr )->get(); break;
        }
        if( c == 0 )
        {
            mode = '@';
    ...
    for( ;; )
    {
        ...
        switch( c )
        {
            case 'D':
                ...
                break;
#ifdef FLOATEMIT
            case 'T': dtostrf( va_arg( ap, double ), 10, 3, ( char * )ptr ); break;
#endif
            case 'H':
            {
#ifdef __AVR__
            ...
            case 'L':
                ...
                break;
            case 'S': strcpy( ( char * )ptr, va_arg( ap, const char * ) ); break;
            case 'F':
            {
            ...
            case 'E':
            {
                /// va_arg를 주석처리해서 E를 사용할 경우 문제가 있을 수 있음
...
uint8_t  EtherCard::destination_ip[4];              // ip address of remote host
uint16_t EtherCard::destination_port       = 80;    // tcp port to browse to
