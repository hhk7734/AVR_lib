...
#define TCPCLIENT_SRC_PORT_H \
    11    //Source port (MSB) for TCP/IP client connections - hardcode all TCP/IP client connection from ports in range 2816-3071
static uint8_t tcpclient_src_port_l
    = 1;    // Source port (LSB) for tcp/ip client connections - increments on each TCP/IP request
static uint8_t tcp_fd;    // a file descriptor, will be encoded into the port
static uint8_t
               tcp_client_state;    //TCP connection state: 1=Send SYN, 2=SYN sent awaiting SYN+ACK, 3=Established, 4=Not used, 5=Closing, 6=Closed
...
static uint8_t ( *client_tcp_result_cb )(
    ...
    uint8_t,
    uint16_t,
    uint16_t );    // Pointer to callback function to handle response to current TCP/IP request
static uint16_t ( *client_tcp_datafill_cb )(
    uint8_t );    //Pointer to callback function to handle payload data in response to current TCP/IP request
static uint8_t
    www_fd;    // ID of current http request (only one http request at a time - one of the 8 possible concurrent TCP/IP connections)
static void ( *client_browser_cb )(
    uint8_t,
    uint16_t,
    uint16_t );    // Pointer to callback function to handle result of current HTTP request
static const char
    *client_additionalheaderline;    // Pointer to c-string additional http request header info
...
static void ( *icmp_cb )(
    uint8_t *
        ip );    // Pointer to callback function for ICMP ECHO response handler (triggers when localhost recieves ping respnse (pong))
static uint8_t destmacaddr
    [6];    // storing both dns server and destination mac addresses, but at different times because both are never needed at same time.
static boolean waiting_for_dns_mac
    = false;    //might be better to use bit flags and bitmask operations for these conditions
...
const unsigned char ntpreqhdr[] PROGMEM
    = { 0xE3, 0, 4, 0xFA, 0, 1, 0, 0, 0, 1 };    //NTP request header
const uint8_t allOnes[] = {
    0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF
};    // Used for hardware (MAC) and IP broadcast addresses
...
/**
 * @brief 출발지, 목적지 맥 주소 설정
 * @param uint8_t *_destination_mac_address
 */
...
/**
 * @brief 출발지, 목적지 맥 주소, IP 설정
 * @param uint8_t *_destination_mac_address
 * @param uint8_t *_destination_ip
 */
...
{
    if( source[0] == 0 || destination[0] == 0 ) { return false; }
    for( int i = 0; i < 4; i++ )
...
{
    return len >= 41 && gPB[ETH_TYPE_H_P] == ETHTYPE_ARP_H_V && gPB[ETH_TYPE_L_P] == ETHTYPE_ARP_L_V
           && memcmp( gPB + ETH_ARP_DST_IP_P, EtherCard::myip, 4 ) == 0;
}

static uint8_t eth_type_is_ip_and_my_ip( uint16_t len )
{
    return len >= 42 && gPB[ETH_TYPE_H_P] == ETHTYPE_IP_H_V && gPB[ETH_TYPE_L_P] == ETHTYPE_IP_L_V
           && gPB[IP_HEADER_LEN_VER_P] == 0x45
           && ( memcmp( gPB + IP_DST_P, EtherCard::myip, 4 ) == 0    //not my IP
                || ( memcmp( gPB + IP_DST_P, EtherCard::broadcastip, 4 )
...
static void make_echo_reply_from_request( uint16_t len )
{
    ...
    if( gPB[ICMP_CHECKSUM_P] > ( 0xFF - 0x08 ) ) gPB[ICMP_CHECKSUM_P + 1]++;
...
static void make_tcp_ack_from_any( int16_t datlentoack, uint8_t addflags )
{
    ...
    if( addflags != TCP_FLAGS_RST_V && datlentoack == 0 ) datlentoack = 1;
...
{    //get the sequence number of packets after an ack from GET
    SEQ = ( ( ( unsigned long )gPB[TCP_SEQ_H_P] * 256 + gPB[TCP_SEQ_H_P + 1] ) * 256
            + gPB[TCP_SEQ_H_P + 2] )
              * 256
          + gPB[TCP_SEQ_H_P + 3];
...
{
    if( is_lan( EtherCard::myip, destip ) ) { setMACandIPs( destmacaddr, destip ); }
    else
...
{
    if( is_lan( myip, ntpip ) ) { setMACandIPs( destmacaddr, ntpip ); }
    else
...
void EtherCard::udpPrepare( uint16_t sport, const uint8_t *dip, uint16_t dport )
{
    if( is_lan(
            myip,
            dip ) )    // this works because both dns mac and destinations mac are stored in same variable - destmacaddr
        setMACandIPs(
            destmacaddr,
            dip );    // at different times. The program could have separate variable for dns mac, then here should be
    else    // checked if dip is dns ip and separately if dip is destination_ip and then use correct mac.
...
void EtherCard::sendUdp(
    const char *data, uint8_t datalen, uint16_t sport, const uint8_t *dip, uint16_t dport )
{
...
{
    if( memcmp( gPB + ETH_ARP_SRC_IP_P, source_ip, 4 ) != 0 ) return 0;
...
{
    for( uint8_t i = 0; i < 4; i++ ) broadcastip[i] = myip[i] | ~netmask[i];
}

static void client_syn( uint8_t srcport, uint8_t dstport_h, uint8_t dstport_l )
{
    if( is_lan( EtherCard::myip, EtherCard::destination_ip ) )
    { setMACandIPs( destmacaddr, EtherCard::destination_ip ); }
    else
        setMACandIPs( gwmacaddr, EtherCard::destination_ip );
    gPB[ETH_TYPE_H_P] = ETHTYPE_IP_H_V;
    gPB[ETH_TYPE_L_P] = ETHTYPE_IP_L_V;
    memcpy_P( gPB + IP_P, iphdr, 9 );
    ...
    if( fd == www_fd )
    {
        if( client_postval == 0 )
        {
            bfill.emit_p( PSTR( "GET $F$S HTTP/1.0\r\n"
                                ...
                                "$F\r\n"
                                "\r\n" ),
                          client_urlbuf, client_urlbuf_var, client_hoststr,
        ...
        else
        {
            ...
            bfill.emit_p( PSTR( "POST $F HTTP/1.0\r\n"
                                ...
                                "\r\n"
                                "$S" ),
                          client_urlbuf, client_hoststr, ahl != 0 ? ahl : PSTR( "" ),
                          ahl != 0 ? "\r\n" : "", strlen( client_postval ), client_postval );
        }
    }
    return bfill.position();
}

static uint8_t www_client_internal_result_cb( uint8_t  fd,
                                              uint8_t  statuscode,
                                              uint16_t datapos,
                                              uint16_t len_of_data )
{
    ...
    else if( statuscode == 0 && len_of_data > 12 && client_browser_cb )
    {
        ...
        ( *client_browser_cb )(
            f, ( ( uint16_t )TCP_SRC_PORT_H_P + ( gPB[TCP_HEADER_LEN_P] >> 4 ) * 4 ), len_of_data );
    }
    return 0;
}
                           ...
                           void ( *callback )( uint8_t, uint16_t, uint16_t ) )
{
    ...
    www_fd = clientTcpReq( &www_client_internal_result_cb, &www_client_internal_datafill_cb,
                           destination_port );
}
...
#if SERIAL
    uart1.transmit( "REQUEST: " );
    uart1.transmit_CR_NL( len );
    uart1.transmit_CR_NL( ( char * )EtherCard::tcpOffset() );
#endif
...
{
    www_fd = clientTcpReq( &tcp_result_cb, &tcp_datafill_cb, destination_port );
    return www_fd;
}

const char *EtherCard::tcpReply( uint8_t fd )
{
    if( result_fd != fd ) return 0;
    result_fd = 123;    // set to a bogus value to prevent future match
    return result_ptr;
}

void EtherCard::registerPingCallback( void ( *callback )( uint8_t *srcip ) ) { icmp_cb = callback; }

uint8_t EtherCard::packetLoopIcmpCheckReply( const uint8_t *ip_monitoredhost )
{
    return gPB[IP_PROTO_P] == IP_PROTO_ICMP_V && gPB[ICMP_TYPE_P] == ICMP_TYPE_ECHOREPLY_V
           && gPB[ICMP_DATA_P] == PINGPATTERN && check_ip_message_is_from( ip_monitoredhost );
}
...
uint16_t EtherCard::accept( const uint16_t port, uint16_t plen )
{
    ...
    if( gPB[TCP_DST_PORT_H_P] == ( port >> 8 ) && gPB[TCP_DST_PORT_L_P] == ( ( uint8_t )port ) )
...
{
    uint16_t len;
    if( using_dhcp ) { ether.DhcpStateMachine( plen ); }

    if( plen == 0 )
    {
        //Check every 65536 (no-packet) cycles whether we need to retry ARP request for gateway
        if( ( waitgwmac & WGW_INITIAL_ARP || waitgwmac & WGW_REFRESHING ) && delaycnt == 0
            && isLinkUp() )
        {
        ...
        {    // send a syn
            ...
            client_syn( ( ( tcp_fd << 5 ) | ( 0x1f & tcpclient_src_port_l ) ), tcp_client_port_h,
                        tcp_client_port_l );
        }
        ...
        //!@todo this is trying to find mac only once. Need some timeout to make another call if first one doesn't succeed.
        if( is_lan( myip, destination_ip ) && !has_dest_mac && !waiting_for_dest_mac )
        {
            client_arp_whohas( destination_ip );
    ...
    {    //Service ARP request
        if( gPB[ETH_ARP_OPCODE_L_P] == ETH_ARP_OPCODE_REQ_L_V ) make_arp_answer_from_request();
        ...
        if( !has_dest_mac && waiting_for_dest_mac
            && client_store_mac( destination_ip, destmacaddr ) )
        {
            has_dest_mac         = true;
    ...
    {    //Service ICMP echo request (ping)
        if( icmp_cb ) ( *icmp_cb )( &( gPB[IP_SRC_P] ) );
    ...
    {    //Source port is in range reserved (by EtherCard) for client TCP/IP connections
        if( check_ip_message_is_from( destination_ip ) == 0 )
        ...
        {    //Waiting for SYN-ACK
            ...
            {                            //Expecting SYN+ACK so reset and resend SYN
                ...
                len++;
                if( gPB[TCP_FLAGS_P] & TCP_FLAGS_ACK_V ) len = 0;
        ...
        {    //TCP connection established so read data
            if( client_tcp_result_cb )
            {
                ...
                if( tcpstart > plen - 8 ) tcpstart = plen - 8;    // dummy but save
                uint16_t save_len = len;
                if( tcpstart + len > plen ) save_len = plen - tcpstart;
                ( *client_tcp_result_cb )( ( gPB[TCP_DST_PORT_L_P] >> 5 ) & 0x7, 0, tcpstart,
    ...
    return accept( destination_port, plen );
}

void EtherCard::persistTcpConnection( bool persist ) { persist_tcp_connection = persist; }
