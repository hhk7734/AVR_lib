...
#include "LOT_enc28j60.h"

uint16_t LOT_enc28j60::bufferSize;
bool     LOT_enc28j60::broadcast_enabled = false;
...
#warning "CS_PIN setting"

static void enableChip() { HAL_GPIO_WritePin( GPIOA, GPIO_PIN_4, GPIO_PIN_RESET ); }

static void disableChip() { HAL_GPIO_WritePin( GPIOA, GPIO_PIN_4, GPIO_PIN_SET ); }
...
static uint8_t readOp( uint8_t op, uint8_t address )
{
    ...
    spi1.transceive( op | ( address & ADDR_MASK ) );
    result = spi1.transceive( 0x00 );
    if( address & 0x80 ) result = spi1.transceive( 0x00 );
...
static void readBuf( uint16_t len, uint8_t *data )
{
    ...
    spi1.transceive( ENC28J60_READ_BUF_MEM );
    while( len-- )
    {
        //xferSPI(0x00);
        //*data++ = SPDR;
        *data++ = spi1.transceive( 0x00 );
    }
    disableChip();
}
...
static void writeBuf( uint16_t len, const uint8_t *data )
{
    ...
    spi1.transceive( ENC28J60_WRITE_BUF_MEM );
    //while (len--)
    //    xferSPI(*data++);

    while( len-- ) spi1.transceive( *data++ );
...
uint8_t LOT_enc28j60::initialize( uint16_t size, const uint8_t *macaddr )
{
    bufferSize = size;
...
bool LOT_enc28j60::isLinkUp() { return ( readPhyByte( PHSTAT2 ) >> 2 ) & 1; }

void LOT_enc28j60::packetSend( uint16_t len )
{
...
uint16_t LOT_enc28j60::packetReceive()
{
    uint16_t len = 0;
...
void LOT_enc28j60::copyout( uint8_t page, const uint8_t *data )
{
...
void LOT_enc28j60::copyin( uint8_t page, uint8_t *data )
{
    uint16_t destPos = SCRATCH_START + ( page << SCRATCH_PAGE_SHIFT );
    if( destPos < SCRATCH_START || destPos > SCRATCH_LIMIT - SCRATCH_PAGE_SIZE ) return;
...
uint8_t LOT_enc28j60::peekin( uint8_t page, uint8_t off )
{
    uint8_t  result  = 0;
...
void LOT_enc28j60::enableBroadcast( bool temporary )
{
    writeRegByte( ERXFCON, readRegByte( ERXFCON ) | ERXFCON_BCEN );
    if( !temporary ) broadcast_enabled = true;
}

void LOT_enc28j60::disableBroadcast( bool temporary )
{
    if( !temporary ) broadcast_enabled = false;
    if( !broadcast_enabled ) writeRegByte( ERXFCON, readRegByte( ERXFCON ) & ~ERXFCON_BCEN );
}

void LOT_enc28j60::enableMulticast()
{
    writeRegByte( ERXFCON, readRegByte( ERXFCON ) | ERXFCON_MCEN );
}

void LOT_enc28j60::disableMulticast()
{
    writeRegByte( ERXFCON, readRegByte( ERXFCON ) & ~ERXFCON_MCEN );
}

uint8_t LOT_enc28j60::doBIST( void )
{
...
#define RANDOM_RACE 0b1100
    ...
    if( ( macResult != bitsResult ) || ( bitsResult != 0xF807 ) ) { return 0; }
    // reset test flag
    writeOp( ENC28J60_BIT_FIELD_CLR, EBSTCON, EBSTCON_TME );

    // Now start the BIST with random data test, and also keep on swapping the
    // DMA/BIST memory ports.
    writeRegByte( EBSTSD, 0b10101010 | time.millis() );
