...
/**
 * @brief *buf_ptr에 데이터 입력 후, 포인터 증가
 * @param uint8_t data
 */
...
static void send_dhcp_message( void )
{
    ...
    if( dhcpState == DHCP_STATE_BOUND ) { EtherCard::copyIp( dhcpPtr->ciaddr, EtherCard::myip ); }
...
static void process_dhcp_offer( uint16_t len )
{
    ...
    do
    {
        ...
        switch( option )
        {
            case 1: EtherCard::copyIp( EtherCard::netmask, ptr ); break;
            case 3: EtherCard::copyIp( EtherCard::gwip, ptr ); break;
            case 6: EtherCard::copyIp( EtherCard::dnsip, ptr ); break;
            case 51:
            case 58:
                leaseTime = 0;    // option 58 = Renewal Time, 51 = Lease Time
                for( uint8_t i = 0; i < 4; i++ ) leaseTime = ( leaseTime << 8 ) + ptr[i];
                leaseTime *= 1000;    // milliseconds
                break;
            case 54: EtherCard::copyIp( EtherCard::dhcpip, ptr ); break;
...
static bool dhcp_received_message_type( uint16_t len, uint8_t msgType )
{
    ...
    if( len >= 70 && gPB[UDP_SRC_PORT_L_P] == DHCP_SRC_PORT && dhcpPtr->xid == currentXid )
    {
        uint8_t *ptr = ( uint8_t * )( dhcpPtr + 1 ) + 4;
        do
        {
...
bool EtherCard::dhcpSetup( const char *hname, bool fromRam )
{
    ...
    if( hname != NULL )
    {
        if( fromRam ) { strncpy( hostname, hname, DHCP_HOSTNAME_MAX_LEN ); }
        else
        {
    ...
    uint16_t start = time.millis();

    while( dhcpState != DHCP_STATE_BOUND && ( uint16_t )( time.millis() - start ) < 60000 )
    {
...
void EtherCard::DhcpStateMachine( uint16_t len )
{
    ...
    if( dhcpState != DHCP_STATE_BOUND )
    {
        uart1.transmit( time.millis() );
        uart1.transmit( " State: " );
    }
    switch( dhcpState )
    {
        case DHCP_STATE_INIT: uart1.transmit_CR_NL( "Init" ); break;
        case DHCP_STATE_SELECTING: uart1.transmit_CR_NL( "Selecting" ); break;
        case DHCP_STATE_REQUESTING: uart1.transmit_CR_NL( "Requesting" ); break;
        case DHCP_STATE_RENEWING: uart1.transmit_CR_NL( "Renew" ); break;
    }
#endif
    ...
    switch( dhcpState )
    {
        case DHCP_STATE_BOUND:
            //!@todo Due to time.millis() 49 day wrap-around, DHCP renewal may not work as expected
            if( time.millis() >= leaseStart + leaseTime )
            {
                send_dhcp_message();
                dhcpState  = DHCP_STATE_RENEWING;
                stateTimer = time.millis();
            }
            break;

        case DHCP_STATE_INIT:
            currentXid = time.millis();
        ...
        case DHCP_STATE_SELECTING:
            ...
            {
                if( time.millis() > stateTimer + DHCP_REQUEST_TIMEOUT )
                { dhcpState = DHCP_STATE_INIT; }
            }
            break;
