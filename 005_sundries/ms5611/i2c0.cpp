...
void i2c0_::read_REG( const uint8_t &addr,
                      ...
                      const uint8_t &count )
{
    ...
    trans_data( regaddr );
    start_SLA_R( addr );
    for( uint8_t i = 0; i < count - 1; ++i ) { *buffer++ = receive_data(); }
    *buffer = last_receive_data();
}

uint8_t i2c0_::get_errors_count( void ) { return errors_count; }
...
void i2c0_::stop( void ) { TWCR = ( 1 << TWINT ) | ( 1 << TWSTO ) | ( 1 << TWEN ); }
