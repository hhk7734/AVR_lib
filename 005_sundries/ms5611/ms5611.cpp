...
void ms5611_::begin( void )
{
    ...
    for( uint8_t i = 0; i < 6; ++i )
    {
        ...
        if( i != 5 )
            C[i] = receive_data() << 8 | receive_data();
        else
            C[i] = receive_data() << 8 | last_receive_data();
    }
}
...
void ms5611_::update( const uint32_t &micros )
{
    switch( sequence )
    {
        case 0:
            ...
            pressure
                = ( SENS * D[1 - 1] / ( ( uint32_t )1 << 21 ) - OFF ) / ( ( uint32_t )1 << 15 );
...
int32_t ms5611_::get_pressure( void ) { return pressure; }

int32_t ms5611_::get_temperature( void ) { return temperature; }
