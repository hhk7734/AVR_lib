...
uint8_t mtk3339_::get_gga_valid( void ) { return gga_valid; }

int32_t mtk3339_::get_latitude( void ) { return latitude; }

int32_t mtk3339_::get_longitude( void ) { return longitude; }

uint16_t mtk3339_::get_altitude( void ) { return altitude; }

uint8_t mtk3339_::get_num_satellite( void ) { return num_satellite; }

uint8_t mtk3339_::get_rmc_valid( void ) { return rmc_valid; }

uint16_t mtk3339_::get_speed( void ) { return speed; }

int16_t mtk3339_::get_gps_direction( void ) { return gps_direction; }
...
uint8_t mtk3339_::parse( void )
{
    uint8_t valid = 0;
    uint8_t i     = available();
    if( i > MAX_PARSING_DATA ) { i = MAX_PARSING_DATA; }
    ...
    for( ; i; --i )
    {
        ...
        else if( data == ',' || data == '*' )
        {
            ...
            {
                case 1:
                    if( string[2] == 'G' && string[3] == 'G' && string[4] == 'A' )
                        parsing_step = FRAME_GGA;
                    else if( string[2] == 'R' && string[3] == 'M' && string[4] == 'C' )
                        parsing_step = FRAME_RMC;
                    break;
