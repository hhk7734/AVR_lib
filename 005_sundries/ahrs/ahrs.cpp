...
void ahrs_::begin( const int16_t *_acc_offset,
                   const int16_t *_g_off,
                   const int16_t *_m_off,
...
void ahrs_::update( const uint16_t &delta_t )
{
    ...
    //
    acc_vector.XYZ.X
        += mul( acc_vector.xyz.y, delta_angle[2] ) - mul( acc_vector.xyz.z, delta_angle[1] );
    acc_vector.XYZ.Y
        += mul( acc_vector.xyz.z, delta_angle[0] ) - mul( acc_vector.xyz.x, delta_angle[2] );
    acc_vector.XYZ.Z
        += mul( acc_vector.xyz.x, delta_angle[1] ) - mul( acc_vector.xyz.y, delta_angle[0] );

    mag_vector.XYZ.X
        += mul( mag_vector.xyz.y, delta_angle[2] ) - mul( mag_vector.xyz.z, delta_angle[1] );
    mag_vector.XYZ.Y
        += mul( mag_vector.xyz.z, delta_angle[0] ) - mul( mag_vector.xyz.x, delta_angle[2] );
    mag_vector.XYZ.Z
        += mul( mag_vector.xyz.x, delta_angle[1] ) - mul( mag_vector.xyz.y, delta_angle[0] );
    //
    ...
    int32_t rawAccSqSum = mul( acc_lpf[0], acc_lpf[0] ) + mul( acc_lpf[1], acc_lpf[1] )
                          + mul( acc_lpf[2], acc_lpf[2] );
    if( 12121538 < rawAccSqSum && rawAccSqSum < 22187868 )
    {
        acc_vector.XYZ.X += ( int32_t )( acc_lpf[0] - acc_vector.xyz.x )
                            << ( 16 - ACCV_COMPLE_FACTOR );
        acc_vector.XYZ.Y += ( int32_t )( acc_lpf[1] - acc_vector.xyz.y )
                            << ( 16 - ACCV_COMPLE_FACTOR );
        acc_vector.XYZ.Z += ( int32_t )( acc_lpf[2] - acc_vector.xyz.z )
                            << ( 16 - ACCV_COMPLE_FACTOR );
    }

    mag_vector.XYZ.X += ( int32_t )( _mag[0] - mag_vector.xyz.x )
                        << ( 16 - MAGV_COMPLE_LPF_FACTOR );
    mag_vector.XYZ.Y += ( int32_t )( _mag[1] - mag_vector.xyz.y )
                        << ( 16 - MAGV_COMPLE_LPF_FACTOR );
    mag_vector.XYZ.Z += ( int32_t )( _mag[2] - mag_vector.xyz.z )
                        << ( 16 - MAGV_COMPLE_LPF_FACTOR );
    ...
    //
    int32_t sqY_sum_sqZ
        = mul( acc_vector.xyz.y, acc_vector.xyz.y ) + mul( acc_vector.xyz.z, acc_vector.xyz.z );
    ...
    //
    int16_t YY
        = ( mul( acc_vector.xyz.z, mag_vector.xyz.x ) - mul( acc_vector.xyz.x, mag_vector.xyz.z ) )
          >> 10;
    int16_t ZZ
        = ( mul( acc_vector.xyz.x, mag_vector.xyz.y ) - mul( acc_vector.xyz.y, mag_vector.xyz.x ) )
          >> 10;
    int32_t sqYY_sqZZ = mul( YY, YY ) + mul( ZZ, ZZ );
    if( YY > 0 )
    {
        rpy[2] = _atan2( ( ( mul( acc_vector.xyz.y, mag_vector.xyz.z )
                             - mul( acc_vector.xyz.z, mag_vector.xyz.y ) )
                           >> 10 ),
