...
void mpu9250_::begin( void )
{
    ...
    write_REG( MPU9250_GYRO_CONFIG,
               0b00011001 );    // FS_SEL +-2000 dps, Filter choice gyro delay 0.11ms
...
void mpu9250_::read_raw_acc( int16_t *xyz ) { read_xyz( MPU9250_ACCEL_XOUT_H, xyz ); }

void mpu9250_::read_raw_gyro( int16_t *xyz ) { read_xyz( MPU9250_GYRO_XOUT_H, xyz ); }
...
void mpu9250_::acc_calibration( void )
{
    ...
    for( uint8_t i = 0; i < 128; ++i )
    {
        ...
        for( uint8_t j = 0; j < 3; ++j ) { raw_acc_sum[j] += raw_acc[j]; }
        _delay_ms( 3 );
    }
...
void mpu9250_::gyro_calibration( void )
{
    ...
    for( uint8_t i = 0; i < 128; ++i )
    {
        ...
        for( uint8_t j = 0; j < 3; ++j ) { raw_gyro_sum[j] += raw_gyro[j]; }
        _delay_ms( 3 );
    }
...
void mpu9250_::mag_calibration( void )
{
    ...
    for( uint8_t k = 0; k < 70; ++k )
    {
        ...
        {
            for( uint8_t j = 0; j < 6; j++ ) { A[i][j] += component[i] * component[j]; }
            b[i] += component[i];
        }
        _delay_ms( 150 );
    }
    ...
    for( uint8_t i = 6; i; i-- )
    {
        uint8_t k    = i - 1;
        parameter[k] = b[k];
        for( uint8_t j = k + 1; j < 6; j++ ) { parameter[k] -= A[k][j] * parameter[j]; }
        parameter[k] /= A[k][k];
    }
...
void mpu9250_::set_offset( const int16_t *_acc,
                           const int16_t *_gyro,
                           const int16_t *_mag,
