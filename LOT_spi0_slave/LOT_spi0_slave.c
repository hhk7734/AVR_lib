...
ISR( SPI_STC_vect )
{
    ...
    if( temp && slave_address_state )
    {
        ...
        if( slave_read_state ) { SPDR = slave_register[slave_register_address]; }
...
void LOT_spi0_slave_begin( void )
{
    ...
    SPCR = _BV( SPIE ) | _BV( SPE ) | ( LOT_SPI0_SLAVE_DATA_ORDER << DORD )
           | ( LOT_SPI0_SLAVE_CLK_IDLE_STATE << CPOL )
...
{
    if( reg_address < LOT_SPI0_SLAVE_REGISTER_LENGTH ) { return slave_register[reg_address]; }
    else
    {
        return -1;
    }
...
{
    if( reg_address < LOT_SPI0_SLAVE_REGISTER_LENGTH ) { slave_register[reg_address] = data; }
}